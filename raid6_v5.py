...
#        print(mat)
    ...
    def read_file_memory(self,file,frm):
        ...
        if frm == '.txt':
            ...
            global SIZE
            im = Image.open(full_path).convert('RGB').resize((SIZE,SIZE),Image.LANCZOS)
            data = asarray(im).flatten().tolist()
    ...
    def fix_failure(self):
        ...
        for i in tqdm(range(self.num_stripes)):    
...
#        self.drives[drive][full_rows,rem] = item
    ...
    def read_file(self,file_id):
        ...
        if file_id.endswith('.png'):
            dim = int(np.sqrt(len(data)/3))
            data = data.reshape((dim,dim,3))
        t2 = time()
        read_time = t2-t1
        return data
...
for i in range(10):
    ...
    for _ in range(3):
        SIZE = (i+1)*25
        cont = Raid6Controller(8,500000,drive_path,file_src)
        cont.detect_new_file()
        #
        sleep(2)
        os.remove(join(drive_path,str(1)+'.npy'))
        os.remove(join(drive_path,str(3)+'.npy'))
        
        cont.detect_failed_drives()
        if cont.failed_drives:
            cont.fix_failure()
        
        im = cont.read_file('image.png')
        
        restore_time_sum += restore_time
        write_time_sum += write_time
        read_time_sum += read_time
        
    restore_times.append(restore_time_sum/3)
    write_times.append(write_time_sum/3)
    read_times.append(read_time_sum/3)
